...
static void initStaticData(void) {
	...
	dispatch_once(&onceToken, ^{
		...
		NSError *error = nil;
		kPrefsPathRegex = [NSRegularExpression
			regularExpressionWithPattern:@"^(/private)?/var/\\w+/Library/Preferences/"
			options:0 error:&error];
	});
}

//–– Decide if a prefs file should be redirected ––
static BOOL preferencePlistNeedsRedirection(NSString *plistPath) {
	initStaticData();
	// Quick path check via regex
	if (![kPrefsPathRegex firstMatchInString:plistPath
									  options:0
										range:NSMakeRange(0, plistPath.length)])
		return NO;
	
	NSString *name = plistPath.lastPathComponent;
	// Whitelist
	if ([kAppleInternalPlists containsObject:name]) return YES;
	// Explicit blacklist prefixes
	for (NSString *prefix in kBlacklistPrefixes) {
...
//–– Hook for CFPreferences path resolution ––
static BOOL (*orig_CFPrefsGetPathForTriplet)(
	CFStringRef, CFStringRef, BOOL, CFStringRef, UInt8*);
BOOL new_CFPrefsGetPathForTriplet(
	CFStringRef identifier, CFStringRef user, BOOL byHost,
	CFStringRef container, UInt8 *buffer)
{
	BOOL result = orig_CFPrefsGetPathForTriplet(
		identifier, user, byHost, container, buffer);
	if (result && buffer) {
		...
		if (preferencePlistNeedsRedirection(origPath)) {
			if (!(gCurrentClientPid > 0 &&
				  jbclient_blacklist_check_pid(gCurrentClientPid)))
			{
				const char *redirected = jbroot(origPath.UTF8String);
				if (strlen(redirected) < MAX_BUFFER_SIZE) {
					strcpy((char*)buffer, redirected);
				} else {
					return NO;
				}
			}
...
//–– Unified daemon message hook ––
static void* (*orig_CFPrefsDaemon_handleMessage)(
	id, xpc_object_t, xpc_connection_t, void*);
void* new_CFPrefsDaemon_handleMessage(
	id self,
	...
	void *replyHandler)
{
	...
	return orig_CFPrefsDaemon_handleMessage(
...
static void cfprefsdInit(void) {
	...
	MSImageRef cfImage = MSGetImageByName(
		"/System/Library/Frameworks/CoreFoundation.framework/CoreFoundation");
	void *sym1 = MSFindSymbol(
		cfImage, "__CFPrefsGetPathForTriplet");
	if (sym1) {
		MSHookFunction(sym1,
	...
	void *sym2 = MSFindSymbol(
		cfImage,
		"-[CFPrefsDaemon handleMessage:fromPeer:replyHandler:]");
	if (sym2) {
		MSHookFunction(sym2,
					   (void*)&new_CFPrefsDaemon_handleMessage,
					   (void**)&orig_CFPrefsDaemon_handleMessage);
	}
