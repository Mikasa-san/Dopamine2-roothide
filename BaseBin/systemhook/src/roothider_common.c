...
// Fast parent PID retrieval with fallback
...
// Extract app UUID path into `out` buffer
static bool extract_uuid_path(const char *path, char *out, size_t outlen) {
	char realp[PATH_MAX];
	if (!path || !realpath(path, realp)) return false;
	const size_t prefix_len = strlen(APP_PATH_PREFIX);
...
static int do_sysctl(int *name, u_int namelen, void *oldp, size_t *oldlenp) {
	...
	if (cached_len && namelen == (u_int)cached_len && memcmp(name, cached_name, namelen * sizeof(int)) == 0) {
